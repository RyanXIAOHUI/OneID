...
![图1 AIPL转化漏斗模型](https://user-images.githubusercontent.com/4908959/177004481-2041491b-e8bb-4912-bc91-bd1656040731.png)
...
![图2 拉链表示例](https://user-images.githubusercontent.com/4908959/177004474-6458347c-3c3a-43b9-9e3a-16093d59aad4.png)
...
![图3 用户主数据表One ID双层数据拉链整合流程图](https://user-images.githubusercontent.com/4908959/177004463-78fbcdca-0b1a-4cba-961a-c8132e42e118.png)
...
![图4 用户主数据拉链表数据集成](https://user-images.githubusercontent.com/4908959/177004448-4d7a30c6-68cc-409f-aefc-802ac4e0ef41.png)
...
- 第二天，从支部宝小程序捕获一条手机号为183xxx、支付宝id为20880001的新数据(new data)，经过与One ID模型中的u001通过手机号183xxx匹配之后，形成一条本批次手机号为183xxx、支付宝id为20880001的待更新数据(update data)；最终，经过拉链操作，形成两条目标数据（target data）：
...
- 第二天，从微信公众号捕获一条手机号为183xxx、微信open id为o12345的新数据(new data)，经过与One ID模型中的u002通过手机号匹配之后，形成一条本批次手机号为183xxx、微信open id为o12345的待更新数据(update data)；最终，经过新型拉链操作，形成三条目标数据（target data）：
  - UUID为u001、手机号为183xxx、微信open id为null、开始日期为2010年01月02日、结束日期为2010年01月03日闭链状态的历史数据，
  - UUID为u002、手机号为null、微信open id为o12345、开始日期为2010年01月02日、结束日期为2010年01月03日闭链状态的历史数据，
  - UUID为u003、手机号为183xxx、微信open id为o12345、支部宝id为20880001、开始日期为2010年01月03日、结束日期为9999年12月31日开链状态的最新数据；
- 与常规数据拉链操作相比，新型数据拉链操作，实现了在同一批次数据处理过程当中，对两条数据记录（u001和u002）同时进行闭链操作、并对一条新的数据记录（u003）进行开链操作，达到用户匹配识别和One ID数据合并整合的目的。

## 2.3 本技术方案带来的有益效果

- 该技术将数据拉链的原理，应用到One ID的建模过程当中，有效利用数据拉链技术来跟踪用户在AIPL模型转化过程中的历史状态运动轨迹，实现转化漏斗分析；
- 该技术应用场景广泛，除了AIPL应用场景之外，例如将核心要素（Primary Element）替换成身份证号，可以延伸应用到集团一体化等场景中；
- 该技方案在传统单层数据拉链基础上进行了创新，在多渠道或集团多子公司的用户信息进行整合时，采用双层数据拉链的方式，在第一层拉链中，有效的标记和跟踪单一批次当中的新数据（new data）和待更新数据（update data）；在第二层拉链中，可以在同一批次的两条记录进行闭链的同时、新增一条合并之后的新记录进行开链，达到识别同一个用户的目标。
- 该技术在数据拉链的处理过程中，start_date和end_date开闭链日期字段定义为int类型，而非date类型，提升开闭链日期的更新和查询的使用效率；
- 该技术可以充分使用类似Hadoop/Hive大数据生态的优势，实现在近万台节点的单集群上进行PB以上级别的大数据处理，无需借助例如图数据库等其它技术平台，降低集群管理的复杂性，并减小管理成本。
